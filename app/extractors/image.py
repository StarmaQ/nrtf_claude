"""
STEG invoice image extractor — Gemini 2.0 Flash backend (free tier).
...
Pipeline per file:
  ...
  2. Classify document subtype via Gemini vision
  3. Extract structured fields via Gemini vision
...
def _load_and_preprocess(image_path: str) -> Image.Image:
    ...
    Returns a PIL Image for the Gemini API.
    """
...
def _call_gemini(image: Image.Image, prompt: str) -> str:
    """Send PIL image + prompt to Gemini, return raw text."""
    response = client.models.generate_content(
        model=MODEL,
        contents=[_SYSTEM_CONTEXT + "\n\n" + prompt, image],
    )
...
def classify_image(image: Image.Image) -> DocumentType:
    try:
        raw = _call_gemini(image, _CLASSIFY_PROMPT)
...
def _extract_bill(image: Image.Image, source_file: str) -> StegBillRecord:
    raw = _call_gemini(image, _BILL_PROMPT)
    data = _parse_json(raw)
...
def _extract_meter_reading(image: Image.Image, source_file: str) -> StegMeterReadingRecord:
    raw = _call_gemini(image, _METER_PROMPT)
    data = _parse_json(raw)
