...
Strategy:
  ...
  2. Fall back to Gemini vision per page (for scanned/image PDFs — all current data)
...
def _extract_page_vision(image: Image.Image, source_file: str, page_num: int) -> AnyRecord:
    """Classify + extract a single rendered PDF page via Gemini vision."""
...
def extract_pdf(pdf_path: str) -> list[AnyRecord]:
    ...
    Uses pdfplumber for text PDFs; Gemini vision for scanned PDFs.
    """
