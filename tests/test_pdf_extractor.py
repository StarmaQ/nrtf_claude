...
Gemini API calls and pymupdf rendering are mocked — no API key or PDF files needed.
"""
import sys, os
...
def test_vision_page_unknown_falls_back_to_energy_record():
    ...
    with patch("app.extractors.pdf.classify_image", return_value=DocumentType.unknown), \
         patch("app.extractors.pdf._call_gemini", return_value=gemini_response):
    ...
    assert rec.extraction_method == "gemini_vision_fallback"


def test_vision_page_unknown_gemini_error_returns_empty_record():
    with patch("app.extractors.pdf.classify_image", return_value=DocumentType.unknown), \
         patch("app.extractors.pdf._call_gemini", side_effect=Exception("timeout")):
