...
def test_classify_steg_bill():
    with patch("app.extractors.image._call_gemini", return_value='{"type": "steg_bill"}'):
        result = classify_image(_DUMMY_IMAGE)
    assert result == DocumentType.steg_bill


def test_classify_meter_reading():
    with patch("app.extractors.image._call_gemini", return_value='{"type": "steg_meter_reading"}'):
        result = classify_image(_DUMMY_IMAGE)
    assert result == DocumentType.steg_meter_reading


def test_classify_unknown_on_error():
    with patch("app.extractors.image._call_gemini", side_effect=Exception("network error")):
        result = classify_image(_DUMMY_IMAGE)
...
def test_extract_bill_fields():
    with patch("app.extractors.image._call_gemini", return_value=_MOCK_BILL_RESPONSE):
    ...
    assert record.extraction_method == "gemini_vision"
...
def test_extract_bill_handles_nulls():
    ...
    with patch("app.extractors.image._call_gemini", return_value=sparse):
...
def test_extract_meter_fields():
    with patch("app.extractors.image._call_gemini", return_value=_MOCK_METER_RESPONSE):
...
def test_meter_net_provided_by_claude():
    ...
    with patch("app.extractors.image._call_gemini", return_value=json.dumps(data)):
        record = _extract_meter_reading(_DUMMY_IMAGE, "releve_feb25.jpeg")
